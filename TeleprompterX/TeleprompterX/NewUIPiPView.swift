...
open class NewUIPiPView: UIView,
    ...
    public lazy var pipController: AVPictureInPictureController? = {
        if NewUIPiPView.isUIPiPViewSupported(), #available(iOS 15.0, *) {
    ...
    public var pipPossibleObservation: NSKeyValueObservation?
    public var frameSizeObservation: NSKeyValueObservation?
    public var refreshIntervalTimer: Timer!
    var isPlaybackPaused = false
    var customPiPView: CustomPiPView?
    ...
    public func initialize() {
        guard NewUIPiPView.isUIPiPViewSupported(),
    ...
    public func startPictureInPictureSub(
    ...
    public func setupVideoLayerView() {
        if (videoLayerView.superview == nil) {
            ...
            frameSizeObservation = self.observe(
                \NewUIPiPView.frame, options: [.initial, .new]) { [weak self] _, _ in
                guard let self = self else { return }
    ...
    open func pictureInPictureControllerDidStartPictureInPicture(
            _ pictureInPictureController: AVPictureInPictureController
        ) {
        }
    ...
    open func pictureInPictureController(
        ...
        // Update the playback state based on the 'playing' parameter
        if playing {
            // If 'playing' is true, it means the playback is active
            // Set the 'isPlaybackPaused' variable to false to resume playback
            isPlaybackPaused = false
            print ("Was paused - is now playing") // Call startScrolling when playback starts
            NotificationCenter.default.post(name: .startScrollingNotification, object: nil)
           
        } else {
            // If 'playing' is false, it means the playback is paused
            // Set the 'isPlaybackPaused' variable to true to pause playback
            isPlaybackPaused = true
            print ("Was playing - is now paused")
            // Call startScrolling when playback starts
            NotificationCenter.default.post(name: .stopScrollingNotification, object: nil)
        }
    }
...
extension Notification.Name {
    static let startScrollingNotification = Notification.Name("StartScrollingNotification")
    static let stopScrollingNotification = Notification.Name("StopScrollingNotification")
}
